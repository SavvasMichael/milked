...
public class MilkingGroupControllerIntegrationTest {
    ...
    public void createGroupReturnsCreatedResponseCode() {
        ...
        assertEquals(1L, (long) groupResponse.getBody().getOwner().getId());
    }
    ...
    // TODO: we down't want to fetch the groups a user owns, we want all the groups a user is a member of
    @Test
    public void checkGetUserGroups() {
        ...
        ResponseEntity<String> createGroupResponse = rest.postForEntity(URI.create(createGroupUrl), groupRequest, String.class);
        ResponseEntity<MilkingGroup> getGroupResponse = rest.getForEntity(URI.create(getGroupUrl + 1), MilkingGroup.class);
        //then
        assertEquals(201, createGroupResponse.getStatusCode().value());
